...
struct CB_Render
{
	...
	float			fWaterSurface_y;
	float			fWaterBottom_y;
	float			niuf[2];
};
...
public:
	...
	Shoal(UINT FishNum, float deltaT = 0.02, XMFLOAT3 centerPos = XMFLOAT3(0, -10, 0),
		  XMFLOAT3 xyzExpand = XMFLOAT3(25, 25, 25), bool bRenderToBackbuffer = false,
		  UINT width = SUB_TEXTUREWIDTH, UINT height = SUB_TEXTUREHEIGHT)
	{
		...
		m_CBsimParams.fSeperationFactor = 0.1;	// Strength of seperation force
		...
		m_CBsimParams.fMinSpeed = 1.5;			// m/s minimum speed a fish have to maintain
	...
	HRESULT Initial(UINT numFish = -1)
	{
		...
		while (i < m_sShoalData.uNInstances){
			...
			x = x*2*m_CBsimParams.f3xyzExpand.x + m_CBsimParams.f3CenterPos.x;
			y = y*2*m_CBsimParams.f3xyzExpand.y + m_CBsimParams.f3CenterPos.y;
			z = z*2*m_CBsimParams.f3xyzExpand.z + m_CBsimParams.f3CenterPos.z;
	...
	HRESULT CreateResource(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dImmediateContext)
	{
		...
		XMVECTOR vecEye = { 0.0f, 0.0f, -100.0f, 0.f };
		XMVECTOR vecAt = { 0.0f, 0.0f, 0.0f, 0.f };
	...
	void Render(ID3D11DeviceContext* pd3dImmediateContext, float waterSurface, float waterBottom,
				XMUINT2 texReso, float cellsize, 
				XMFLOAT3 lightDir, ID3D11ShaderResourceView* causticSRV,
				D3D11_VIEWPORT &vp, XMMATRIX &mViewProj, XMFLOAT3 eyePos)
	{
		HRESULT hr = S_OK;
		
		// Updating rendering constant buffer
		CB_Render CBRender;
		CBRender.mWorldViewProj = XMMatrixTranspose(mViewProj);
		CBRender.fSize = m_fFishSize;
		CBRender.fMinSpeed = m_CBsimParams.fMinSpeed;
		CBRender.fMaxSpeed = m_CBsimParams.fMaxSpeed;
		CBRender.f3LightDir = lightDir;
		CBRender.f3EyePos = eyePos;
		CBRender.iTexReso = texReso;
		CBRender.fCellSize = cellsize;
		CBRender.fWaterSurface_y = waterSurface;
		CBRender.fWaterBottom_y = waterBottom;
		...
		pd3dImmediateContext->PSSetConstantBuffers(0, 1, &m_pRenderCBDraw);
		...
		pd3dImmediateContext->PSSetShaderResources(3, 1, &causticSRV);
