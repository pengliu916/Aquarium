...
void CS(uint3 Gid : SV_GroupID, uint3 DTid : SV_DispatchThreadID, uint3 GTid : SV_GroupThreadID, uint GI : SV_GroupIndex)
{
	...
	for (uint tile = 0; tile <= uint(uNumInstance / blocksize); tile++)
	{
		...
		for (uint counter = 0; counter < blocksize; counter++)
		{
			...
			if(tile*blocksize + counter == DTid.x) notMySelf = 0; 
			// Doing one to one interaction based on visibility
			if ((dist <= vDT && cosAngle >= vAT)||!notMySelf)
			{
				...
				accForce += Seperation(neighborDir, sharedFishVP[counter].vel, invDist)*predatorFactor*notMySelf;
				accForce += Avoidance(localVP.pos, velDir, sharedFishVP[counter].pos, distSqr)*predatorFactor*notMySelf;
#if PREDATOR
	...
	if (accCount != 0){
		...
		accForce += Cohesion(localVP.pos, avgPos-normalize(avgVel)*0.1);
